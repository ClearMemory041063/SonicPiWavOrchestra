...
To shift up and octave play the sound twice as fast, to shift down an octave play the sound at half speed.
So heres a function that returns the speed multiplier needed to shift N ocataves.

```
#getoactavemultiplier
# define the function
def getoctave (a)
  b=0.0
  if(a>0.0)
...
So let's insert the multiplier function into the playwavetune program, and add a second voice with the octave shifted.
With the program runn9ing, try changing the variable oshift, and clicking on play
playwave2.txt
